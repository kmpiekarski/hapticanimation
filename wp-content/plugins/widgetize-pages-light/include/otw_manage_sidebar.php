...
/** Create/edit otw sidebar
  ...
  */
	...
	$page_title = __( 'Create New Sidebar', 'widgetize-pages-light' );
	...
	if( isset( $_GET['sidebar'] ) ){
		...
		if( is_array( $otw_sidebars ) && isset( $otw_sidebars[ $otw_sidebar_id ] ) && ( $otw_sidebars[ $otw_sidebar_id ]['replace'] == '' )  ){
			...
			$page_title = __( 'Edit Sidebar', 'widgetize-pages-light' );
		}
	}
	//apply post values
				...
				<div id="post-body-content">
					...
					<div id="col-left">
						<div class="form-field form-required">
							<label for="sbm_title"><?php _e( 'Sidebar title', 'widgetize-pages-light' );?></label>
							<input type="text" id="sbm_title" value="<?php echo $otw_sidebar_values['sbm_title']?>" tabindex="1" size="30" name="sbm_title"/>
							<p><?php _e( 'The name is how it appears on your site.', 'widgetize-pages-light' );?></p>
						</div>
						<div class="form-field">
							<label for="sbm_status"><?php _e( 'Status', 'widgetize-pages-light' );?></label>
							<select id="sbm_status" tabindex="2" style="width: 170px;" name="sbm_status">
								...
								<option value="active"<?php if( $otw_sidebar_values['sbm_status'] == 'active' ){ echo ' selected="selected" ';}?>><?php _e( 'Active', 'widgetize-pages-light' )?></option>
								<option value="inactive"<?php if( $otw_sidebar_values['sbm_status'] == 'inactive' ){ echo ' selected="selected" ';}?>><?php _e( 'Inactive', 'widgetize-pages-light' )?></option>
							</select>
						</div>
						<div class="form-field">
							<label for="sbm_description"><?php _e( 'Description', 'widgetize-pages-light' )?></label>
							...
							<p><?php _e( 'Short description for your reference at the admin panel.', 'widgetize-pages-light')?></p>
						</div>
						<p class="submit">
							<input type="submit" value="<?php _e( 'Save Sidebar', 'widgetize-pages-light') ?>" name="submit" class="button"/>
						</p>
					</div>
				</div>
			</div>
